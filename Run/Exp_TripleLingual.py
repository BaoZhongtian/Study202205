...
def Treatment(batch):
    ...
    portuguese_input_ids = tokenizer.batch_encode_plus(
        ['Summarize Portuguese to English :' + portuguese_article], max_length=2048, return_tensors='pt',
        truncation=True)['input_ids']
    spanish_input_ids = tokenizer.batch_encode_plus(
        ['Summarize Spanish to English :' + spanish_article], max_length=2048, return_tensors='pt',
        truncation=True)['input_ids']
    summary_ids = tokenizer.batch_encode_plus([english_summary], return_tensors='pt')['input_ids']

    if cuda_flag:
        portuguese_input_ids, spanish_input_ids, summary_ids = portuguese_input_ids.cuda(), spanish_input_ids.cuda(), summary_ids.cuda()
    return portuguese_input_ids, spanish_input_ids, summary_ids
...
if __name__ == '__main__':
    ...
    save_path = 'mT5_TripleLingual_LateFusion_Project/'
